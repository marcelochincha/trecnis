...
static void build_city(Game* e) {
    ...
    load_obj_tris("res/models/sculpture.obj", vec3(0,1.15f,0), 0.8f, vec3(0.96f,0.78f,0.35f), 0.15f, tris, 1.0f);
...
static void build_cornell_box(Game* e) {
    e->peds.clear();
    std::vector<bvh::Tri> tris;
    load_obj_tris("res/cornell/CornellBox-Original.obj", vec3(0.0f, 1.5f, 0.0f), 3.0f,
...
static void build_character(Game* e) {
    ...
    load_obj_tris("res/mall/mall.obj", vec3(0, 0, 0), 1.0f, vec3(0.8f, 0.8f, 0.8f), 0.5f, tris);
    rebuild_field_mesh(e, tris);
    e->static_bvh.build(std::move(tris), e->build_strategy);
    //e->cam.setFov(40.0f);
